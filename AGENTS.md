...
- Hunter Pi `0.1.0-dev.1` pins Pi `0.84.1`. Exact source `d9f2d931b9fc42d23ceae60fada2aee811caf2ec` passed main CI `31643808274`; unsigned prerelease `v0.1.0-dev.1` publishes the Windows x64 ZIP, checksum, and PowerShell installer. Actual GitHub URL installation, Provider-neutral update/rollback, disposable-fixture inspection, and offline TUI startup passed. No Pi 0.84.1 Provider request or real-user-repository use was performed, so new daily-use acceptance remains `NOT_PROVEN`.
- Do not claim general plugin compatibility, signed/Stable distribution, production readiness, or a new Pi 0.84.1 daily-use acceptance until their later tasks and Evidence are complete. Do not silently expand to a Pi or Oh My Pi fork.
