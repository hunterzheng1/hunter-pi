...
- 当前 `0.1.0-dev.1` 本地完整门禁通过 73 个测试文件、745 个测试，以及 lint、类型检查、严格编译、构建、格式、13 个外部包 smoke、干净安装和 Pi `0.84.1` 公共接口探针；历史 `0.83` 日用 Evidence 由严格 v1 读取合同保持可读，但不能据此判定新版本日用 `GO`。
