...
**Task 6 已在有界范围内合并并通过精确 main 双平台与聚合 Evidence 门禁；Task 7 的 worktree、租约和受管进程安全层已通过本地 Windows/Ubuntu 临时 fixture，但远端 CI 仍为 `PENDING`。Task 7 合并且后续入口明确授权前，仍不得把 Managed Change 用于真实项目。**
