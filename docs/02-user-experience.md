...
```powershell
hpi change --repo D:\Pilot\sample --plan .\task-02.json --run-archive-id archive-task-02 --pilot-interruption FORCED_PROCESS_KILL --json --allow-provider-request
hpi change --repo D:\Pilot\sample --plan .\task-06.json --run-archive-id archive-task-06 --pilot-interruption TERMINAL_CLOSE_SIMULATION --json --allow-provider-request
hpi change --repo D:\Pilot\sample --plan .\task-10.json --run-archive-id archive-task-10 --pilot-interruption POWER_LOSS_SIMULATION --json --allow-provider-request
```
