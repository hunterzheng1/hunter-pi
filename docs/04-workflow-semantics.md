...
Windows portable qualification uses the same rule as update activation. `QUALIFY` binds a path-free release identity and exact GitHub Actions run. Qualification and CLI apply/rollback share the installation-owned manager journal under `.hpi-update/manager`; user-profile state never splits those proofs. Before changing either candidate metadata file, the adapter persists an intent containing the original operation identity, request fingerprint, generated candidate, and strict qualification Evidence; Evidence is synced and atomically published without replacing an existing identity. The intent remains until the manager has durably appended the original APPLIED Receipt and the adapter has verified that exact Receipt binding. If no operation Receipt existed before interruption, reconciliation verifies both artifact copies and the active pointer, completes or rejects the intent, and publishes the original operation Receipt exactly once. If the Receipt exists but intent acknowledgement was interrupted, reconciliation removes only the verified intent and does not append a duplicate Receipt. If a prior `UNKNOWN` Receipt exists in another operation family, it remains immutable and reconciliation appends a separate result.
