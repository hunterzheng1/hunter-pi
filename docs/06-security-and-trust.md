...
- Promotion and production CLI update operations use the same installation-owned `.hpi-update/manager` journal; qualification identity and prior-active proof cannot be split across the portable root and a user profile.
- Every Windows portable rollback candidate, whether recovered from APPLY history or from the initial-install fallback, is re-read from the installed version and must still bind its exact retained qualification Evidence before restoration. A fallback also requires the exact earlier `QUALIFY/APPLIED` candidate identity that preceded the APPLY Receipt proving it was active; a release ID alone is insufficient.
