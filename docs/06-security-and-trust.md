...
- Every Windows portable rollback candidate, whether recovered from APPLY history or from the initial-install fallback, is re-read from the installed version and must still bind its exact retained qualification Evidence before restoration.
