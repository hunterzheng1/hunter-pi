...
Task 7's earlier local receipts are preserved but superseded after independent review reproduced source-loss, lease-race, process-escape, and verifier-binding gaps. Local remediation covers executable Git integrations, ignored files, byte-level source snapshots, post-create compensation, mismatch receipts, one-transaction lease publication, atomic session binding, Linux subreaper descendant tracking, and literal Windows exit code 259. Replacement local v2 Evidence passes; independent rereview and remote CI remain `PENDING` as recorded in the Task 7 validation report.
