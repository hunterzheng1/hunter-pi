...
The example retains the historical `0.1.0-dev.1` pre-qualification state. The published dev.1 release completed its recorded gates; current `0.1.0-dev.2` source has a separate pending publication and real-machine acceptance. Historical Distribution Releases and Evidence retain their original identities.
...
Provide a Windows x64 installer or portable artifact. Hunter Pi `0.1.0-dev.2` packages the existing versioned portable layout as `hpi-windows-x64.zip` and publishes the single maintained `scripts/install.ps1` both inside the ZIP and as a standalone Release asset. The artifact must:
...
Task 11's original portable directory remains historical. The `0.1.0-dev.2` ZIP and PowerShell entry point remain intentionally unsigned `developer-preview`; checksums prove integrity, not publisher identity. Normal users run `hpi update` for official-channel discovery, strict download verification and transactional apply, and `hpi update status` for a readable summary. The file-bearing `check` and `apply` commands remain advanced offline/Evidence interfaces. Because `0.1.0-dev.1` predates the user-level update client, the dev.2 installer has one explicit bootstrap exception: an exact dev.1 installation may consume a promoted dev.2 source with the same Pi Engine and retained hosted qualification Evidence, using the incoming dev.2 update manager for apply and rollback. Health or PATH failure restores dev.1, and replay after success is idempotent. Every other differing active release fails closed and directs the operator to `hpi update`; future installer versions must not silently broaden this one-time migration. Windows CI builds and installs the unqualified portable plus ZIP/checksum/installer snapshot. Only after exact main CI and promotion may the release operator rebuild the same snapshot with the candidate, bundle, and qualification Evidence update assets. Ubuntu validates provider-neutral and cross-platform contracts but does not emit a Windows package.

The recommended installer entry is the versioned Release asset `https://github.com/hunterzheng1/hunter-pi/releases/download/<tag>/install.ps1`; the mutable `main` script remains an inspectable development entry, not the default user path. The one-copy command verifies the installer against a release-specific SHA-256 before execution. The promoted Release contains `install.ps1`, `hpi-windows-x64.zip`, `hpi-windows-x64.zip.sha256`, `portable-release-candidate.json`, `update.bundle.tgz`, and `windows-portable-qualification-evidence.json`. The last three assets are consumed only by the official-channel update client and must come from the same frozen promoted portable snapshot.
