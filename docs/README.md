...
- Product status: **TASK_6_MERGED_MAIN_CI_PASS / TASK_7_REMEDIATION_ACTIVE / PRIOR_TASK_7_EVIDENCE_SUPERSEDED**
- Current implementation: Tasks 1–6 are merged through the bounded disposable-fixture Managed Change milestone; Task 7 is remediating independently reproduced source-loss, lease-race, process-escape, and Evidence-identity gaps
...
- Task 7 earlier local platform Evidence: **SUPERSEDED for completion** — the immutable receipts remain preserved, including the first `NOT_PROVEN` parse, but independent review showed that their matrix and source digest omitted required hard-stop cases and verifier inputs. Replacement Evidence and remote CI remain `PENDING`.
