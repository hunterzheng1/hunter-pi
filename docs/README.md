...
- Task 12 pilot plan/evaluator: **IMPLEMENTED / real pilot NOT_RUN** — strict ten-task, interruption, performance, plugin, privacy, storage, explicit PASS fresh-install receipt, fingerprint-bound explicit repository/provider plan, safe `hpi pilot preflight --plan <file> --json` receipt, two-repository/source-identity comparator, source-bound Windows/Ubuntu CI run receipts, and CI aggregation returns `NOT_PROVEN` when required evidence is missing. A synthetic GO fixture tests only evaluator logic and is not product Evidence. Local verification is 47 files / 371 tests.
