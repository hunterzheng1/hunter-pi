...
- Current repository phase: `TASKS_0_12_COMPLETE_WITHIN_RECORDED_BOUNDS / REAL_WINDOWS_DAILY_USE_GO`
- Active evidence gate: `NONE_WITHIN_UNSIGNED_WINDOWS_DEVELOPER_PREVIEW_BOUNDARY`
...
Implementation status on 2026-08-08: **HARDENED IMPLEMENTATION / FOCUSED LOCAL PASS / CLEAN-COMMIT PLATFORM EVIDENCE PENDING WITHIN RESOURCE-ONLY PACKAGE BOUNDS** — the fixed public Pi PackageManager resolves exact LOCAL and selected Pi-import fixtures, while NPM/Git exact-source behavior is separately labelled as adapter-contract proof unless a real public install actually runs. A bounded installed-artifact public npm observation passed on the current Windows development machine, including quarantine and managed removal; public Git and lifecycle-attack installation remain `NOT_RUN` in platform Evidence. Installation uses a sanitized child process with finite time/output/tree/free-space budgets and cleans failed generations. Two locked external Pi examples resolve without code evaluation, v1 history remains replay-compatible, v2 portable Manifests are separated from device-local bindings, and exact skill/prompt/theme resources activate from Hunter-owned read-only content snapshots after startup revalidation. The CLI supplies list/install/import/disable/remove flows; removal deletes only validated managed snapshots/bindings and preserves append-only history. Five malicious fixture classes force Safe Mode with zero extension evaluation. Executable extensions remain `UNVERIFIED` and quarantined, `.git`/`node_modules` are outside the resource-only tree fingerprint, and neither general third-party compatibility nor OS containment against same-authority mutation is claimed. Exact Windows/Ubuntu Task 10 receipts and their comparator remain `PENDING` until hosted CI runs on the final clean commit.
...
Implementation status on 2026-08-06: **PLAN/PREFLIGHT/EVALUATE CLI IMPLEMENTED / LOCAL PASS / PR CI PASS / MAIN CI PASS AFTER FAILED-JOB RERUN / REAL PILOT NOT_RUN** — the strict v4 Evidence schema with explicit no-manual-state-edit receipt and false-READY zero-tolerance STOP, explicit PASS fresh-install receipt, source-bound and platform/run-distinct Windows/Ubuntu CI run receipts, frozen machine/comparator/check/plugin/update inputs, target-reference and acceptance-definition binding, two-distinct-repository/source-identity comparator, exact-plan Evidence binding, fingerprint-bound ten-task plan compiler, explicit Provider-request authorization gate, path-free machine identity validation, safe `hpi pilot compile`, `hpi pilot preflight`, and `hpi pilot evaluate` CLI commands with redacted/path-free contract output, and GO/REVISE/STOP/NOT_PROVEN evaluator policy tests. The Task 12 CLI hardening merge head `e886b48a617d6df8f63bbaae9cadcc0ea0f66dba` passes 47 files / 391 local tests; [PR CI `31082027466`](https://github.com/hunterzheng1/hunter-pi/actions/runs/31082027466) and exact [main CI `31083253052`](https://github.com/hunterzheng1/hunter-pi/actions/runs/31083253052) pass Windows, Ubuntu, containment, and Evidence gates. Documentation merge `8324ca74604472468bc2c42c5e924663e57a61a4` retained the first Task 7 containment failures in main run `31085901324`; the failed Windows/Ubuntu jobs were rerun without source changes and passed, followed by the Task 7 Evidence aggregation. The policy requires either at least 30% aggregate manual-intervention reduction or a contained false completion, while median Hunter-only overhead remains an independent ≤10-minute gate. No repository targets, Provider credentials, or ten-task observations were safely inferable; daily-use acceptance remains `NOT_PROVEN` until the real pilot produces its required Archive.

Task 12 closure addendum (2026-08-12): the immutable real Windows pilot Archive completed 10/10 tasks in two disposable repositories, recovered all three planned interruptions in the same Runs, passed update/rollback, adversarial-plugin, privacy/storage, Provider-accounting, performance, and independent-comparator gates, and evaluates to `GO` under the test-first 3.5-second Windows warm-start criterion. PR #83 run [`31582820744`](https://github.com/hunterzheng1/hunter-pi/actions/runs/31582820744) and exact merged-head main run [`31584966554`](https://github.com/hunterzheng1/hunter-pi/actions/runs/31584966554) passed all six required jobs. This closes Task 12 only within the unsigned Windows developer-preview boundary; signing, Stable publication, broader plugin qualification, physical power-loss testing, non-Windows daily-use acceptance, and arbitrary real-user-repository safety remain separately scoped.
