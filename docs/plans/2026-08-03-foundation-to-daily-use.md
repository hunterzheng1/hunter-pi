...
Implementation status on 2026-08-06: **LOCAL PASS / REVIEW-HARDENED PR CI PENDING WITHIN UNSIGNED DEVELOPER-PREVIEW BOUNDS** — qualification runner, Evidence-bound checks, Stable/Preview/self-update gates, request-fingerprint replay, exact artifact digest/health/apply/rollback journal, fail-closed portable redaction for failure receipts, Windows x64 portable directory with embedded Node 24, external package smoke, clean install smoke, and Pi probe pass. Earlier PR CI run `31032218373` covers the pre-hardening source; replacement PR CI is pending. The current review-worktree portable manifest is intentionally `sourceState=DIRTY` until the hardened source is committed and repacked; the resulting artifact remains unsigned with `updateChannel=developer-preview`. Exact main CI, publication, signing, state migration, and stable promotion remain `NOT_PROVEN`/`PENDING`.
