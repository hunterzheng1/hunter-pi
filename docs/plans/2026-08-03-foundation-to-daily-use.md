...
Implementation status on 2026-08-06: **PLAN/PREFLIGHT/EVALUATE CLI IMPLEMENTED / LOCAL PASS / PR CI PENDING / MAIN CI PENDING / REAL PILOT NOT_RUN** — the strict v4 Evidence schema with explicit no-manual-state-edit receipt and false-READY zero-tolerance STOP, explicit PASS fresh-install receipt, source-bound and platform/run-distinct Windows/Ubuntu CI run receipts, frozen machine/comparator/check/plugin/update inputs, target-reference and acceptance-definition binding, two-distinct-repository/source-identity comparator, exact-plan Evidence binding, fingerprint-bound ten-task plan compiler, explicit Provider-request authorization gate, path-free machine identity validation, safe `hpi pilot compile`, `hpi pilot preflight`, and `hpi pilot evaluate` CLI commands with redacted/path-free contract output, and GO/REVISE/STOP/NOT_PROVEN evaluator policy tests. The v4 hardening merge head `647806b20c84143bcf5c57d812d9ce588f7ad72d` passes 47 files / 391 local tests on this branch; the prior merged v4 code is covered by [PR CI `31071253465`](https://github.com/hunterzheng1/hunter-pi/actions/runs/31071253465) and exact [main CI `31072260307`](https://github.com/hunterzheng1/hunter-pi/actions/runs/31072260307). The policy requires either at least 30% aggregate manual-intervention reduction or a contained false completion, while median Hunter-only overhead remains an independent ≤10-minute gate. Each CI run retains its initial Ubuntu hosted-test timeout and one failed-job rerun without source changes. No repository targets, Provider credentials, or ten-task observations were safely inferable; daily-use acceptance remains `NOT_PROVEN` until the real pilot produces its required Archive.
