...
- Managed review now fingerprints Git-visible changed content and existing ignored content, freezes the exact interrupted/fixback workspace across recovery, and compares the workspace before and after every independent Verification command. An ignored-file mutation, verification side effect, or recovery-window drift blocks readiness;
