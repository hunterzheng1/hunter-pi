...
Tasks 0–12 are complete within their recorded Pi 0.83 bounds. Task 12 has one complete immutable real Windows Archive over two disposable repositories: 10/10 tasks, all three same-Run interruption recoveries, three raw-Pi comparators, two update/rollback cycles, five plugin fixtures, privacy/storage gates, and exact source CI receipts. The unchanged Evidence first produced `REVISE` at the original 3-second warm-start target and produces `GO` under the documented 3.5-second Windows security-software variance target. Its historical complete local gate passed 71 files / 727 tests; PR #83 run `31582820744` and exact merged-head main run `31584966554` passed all six required jobs.

Hunter Pi `0.1.0-dev.1` pins Pi `0.84.1` and adds the Windows ZIP plus single-source PowerShell installer. Its clean-first local gate passes 73 files / 748 tests. Exact main CI `31643808274`, prerelease assets, published-URL installation, Provider-neutral update/rollback, and offline TUI startup passed. A new real Provider and daily-use acceptance remain separate gates; the Pi 0.83 Archive cannot satisfy them.
