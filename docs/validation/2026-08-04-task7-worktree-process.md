...
- Task result: **COMPLETE WITHIN DISPOSABLE-FIXTURE BOUNDS / PLATFORM V4 PASS / FULL LOCAL PASS / EXACT-HEAD REREVIEW PASS / PR CI PASS / MAIN CI PASS / TASK 8 NOT_STARTED**
...
An August 11 daily-use replacement verification exposed that the clean-worktree precondition still inherited ambient `core.excludesFile`. On the local restricted Windows runner, the default user ignore path intermittently emitted a permission warning that the strict command gate correctly rejected; independently, a configured ambient exclude could hide an unrelated untracked file. A RED fixture reproduced the hidden-file false clean. The probe now overrides only that ambient exclude source with command-scoped `core.excludesFile=/dev/null` for its status observation. Git for Windows and Ubuntu both support this path, repository ignore semantics remain unchanged, and the focused Task 7 platform file passes 11/11 while proving that ambient excludes cannot hide the blocking file.

The subsequent compiled qualification probe exposed a second ambient Windows boundary: the parent process contained both `PATH` and `Path`. Their values were equal, but PowerShell 7 rejects JSON objects with keys that differ only by case before the Job helper can establish containment. A Windows RED reproduced the failure with an innocuous duplicate environment name. The driver now sorts the schema-restricted ASCII names and keeps the first case-insensitive match, matching Node's Windows child-environment selection while ensuring the helper receives unambiguous JSON. Independent review then caught that placing this tenth test in the frozen nine-check platform file would make the formal Evidence parser reject both CI reports. The regression now lives in a separate Windows quality file: it passes 1/1, while the Task 7 platform matrix remains exactly 9/9. This does not weaken process finality or add a shell.
...
PR #16 replacement run [`30977216739`](https://github.com/hunterzheng1/hunter-pi/actions/runs/30977216739) completed `success` for exact head `8e7086ddad86da3c6dc75f9934bd7853bc4554ba` after one append-only failed-job rerun. Its initial Ubuntu Task 7 containment job emitted a structured `TEST_EXECUTION` failure (exit 1; downstream aggregate dependency-skipped), while Windows/Ubuntu quality, Windows containment, and Pi identity checks passed. The same failed job rerun [`92216076192`](https://github.com/hunterzheng1/hunter-pi/actions/runs/30977216739/job/92216076192) passed without source changes, followed by the Task 7 identity aggregate [`92216198047`](https://github.com/hunterzheng1/hunter-pi/actions/runs/30977216739/job/92216198047); all PR checks are now green. The initial structured failure is retained as non-reproducible host/test-scheduling history, not rewritten as a PASS.

PR #16 merged as `91500066b3e809b92b0ab0e985adb165b2276746`. Main run [`30979052589`](https://github.com/hunterzheng1/hunter-pi/actions/runs/30979052589) first recorded a real Ubuntu quality-job failure: the nested process-tree reconciliation test timed out at its case-owned 15-second budget with 298/302 tests passed and 3 skipped; the raw failure is retained and not rewritten. A single `--failed` rerun, without source changes, produced successful Ubuntu quality, Windows quality, both containment jobs, Task 7 identity, and Pi identity aggregation; the exact merge head therefore completed main CI `success`. This closes Task 7 only within automatically created disposable fixtures. The v4 Evidence envelopes retain `remoteCi=PENDING` because that was their emission-time value, and no real repository or Provider request was used.
