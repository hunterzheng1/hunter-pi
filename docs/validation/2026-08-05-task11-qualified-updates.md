...
No npm publication, Windows installer/signing, migration of existing user state, or stable promotion occurred. Earlier PR CI run `31032218373` covers the pre-hardening source; hardened replacement PR CI [`31042109585`](https://github.com/hunterzheng1/hunter-pi/actions/runs/31042109585) passes the Windows/Ubuntu quality, containment, and Evidence aggregators, while exact main CI remains `PENDING`. The initial fresh-checkout updater lint failure is preserved in `31022928024`, and the later hosted Linux process-tree history is preserved in `31028497285`. The clean unsigned artifact and these PR-only CI results still do not establish a qualified release claim.
